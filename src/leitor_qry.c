...
// Estrutura auxiliar para componentes conexos
...
typedef struct componentes_conexos {
    int *visitado;
    int n_vertices;
    Grafo *g;
    int num_componentes;
...
static void dfs(ComponentesConexos *comp, int v, int *comp_id, int id) {
    comp->visitado[v] = 1;
    comp_id[v] = id;
    
    Vertice *vert = grafo_get_vertice(comp->g, v);
    Aresta *a = vertice_get_primeira_aresta(vert, comp->g);
    while (a) {
        if (aresta_get_velocidade(a) < 5.0) {  // velocidade baixa
            Vertice *dest = aresta_get_destino(a);
            int w = vertice_get_indice(dest);
            if (!comp->visitado[w]) {
                dfs(comp, w, comp_id, id);
            }
        }
        a = aresta_get_prox(a);
    }
...
static int calcular_componentes_conexos(Grafo *g, double vl, int **comp_id) {
    ...
    *comp_id = (int*)calloc(n, sizeof(int));
    ...
    for (int i = 0; i < n; i++) {
        if (!visitado[i]) {
            // Verificar se vértice tem alguma aresta com velocidade < vl
            Vertice *v = grafo_get_vertice(g, i);
            Aresta *a = vertice_get_primeira_aresta(v, g);
            int tem_aresta_lenta = 0;
            while (a) {
            ...
            if (tem_aresta_lenta) {
                // Inicia nova componente conexa
                num++;
                // Fazer DFS limitado a arestas lentas
...
static void calcular_agm_velocidade_lenta(Grafo *g, double vl, SVG *svg) {
    ...
    // Coletar arestas com velocidade < vl
    ArestaAGM *arestas = (ArestaAGM*)malloc(grafo_num_arestas(g) * sizeof(ArestaAGM));
    ...
    // Desenhar arestas selecionadas e aumentar velocidade
...
static void processar_comando_oreg(ContextoQRY *ctx, char *linha) {
    ...
    if (sscanf(linha, "@o? %d %s %c %d", &reg, cep, &face, &num) == 4) {
        ...
        if (q) {
            ...
            svg_linha(ctx->svg, x, 0, x, ctx->altura_svg, "red", 1.0, "5,5");
            svg_texto(ctx->svg, x + 2, 15, (char[2]){reg + '0', '\0'}, "red", 10);
            
            fprintf(ctx->txt_output, "[*] @o? %d %s %c %d\n", reg, cep, face, num);
            fprintf(ctx->txt_output, "Coordenada: (%.2f, %.2f)\n", x, y);
        }
    }
}

static void processar_comando_mv(ContextoQRY *ctx, char *linha) {
    double x, y, w, h, v;
    if (sscanf(linha, "mv %lf %lf %lf %lf %lf", &x, &y, &w, &h, &v) == 5) {
        grafo_atualizar_velocidade_regiao(ctx->grafo, x, y, w, h, v);
        fprintf(ctx->txt_output, "[*] mv %.2f %.2f %.2f %.2f %.2f\n", x, y, w, h, v);
...
static void processar_comando_regs(ContextoQRY *ctx, char *linha) {
    ...
    if (sscanf(linha, "regs %lf", &vl) == 1) {
        ...
        for (int c = 1; c <= n_comps; c++) {
            ...
            if (min_x < 1e8) {
                double w = max_x - min_x + 10;
                double h = max_y - min_y + 10;
                svg_bounding_box(ctx->svg, min_x - 5, min_y - 5, w, h, 
                                cores[(c-1) % 10], 0.5);
            }
        }
...
static void processar_comando_p(ContextoQRY *ctx, char *linha) {
    ...
    if (sscanf(linha, "p? %d %d %s %s", &reg1, &reg2, cc, cr) == 4) {
        ...
        if (curto) {
            ...
            // Animação
            char path_data[1024] = "";
            for (int i = 0; i < caminho_num_vertices(curto) - 1; i++) {
                Vertice *v1 = caminho_get_vertice(curto, i);
                Vertice *v2 = caminho_get_vertice(curto, i + 1);
                char ponto[128];
                sprintf(ponto, "M%.2f,%.2f L%.2f,%.2f ", 
                        vertice_get_x(v1), vertice_get_y(v1),
                        vertice_get_x(v2), vertice_get_y(v2));
                strcat(path_data, ponto);
            }
            svg_animate_motion(ctx->svg, "circle", 5, 0, 0, "blue", "blue", path_data, 3.0);
            
            // Descrição textual do percurso
...
int executar_arquivo_qry(const char *nome_arquivo, ContextoQRY *ctx, const char *nome_base_svg) {
    ...
    while (fgets(linha, MAX_LINE, f)) {
        ...
        else if (strncmp(linha, "mv", 2) == 0) {
            processar_comando_mv(ctx, linha);
        }
